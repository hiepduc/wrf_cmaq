...
if nc_path:
    try:
        ...
        # 🔼 Put these just after loading the CMAQ file and extracting times
        show_wind = st.sidebar.checkbox("Show Wind Overlay", value=True)
        #play_time = st.sidebar.slider("Select Time Step", 0, len(times)-1, 0, 1)
        #auto_play = st.sidebar.checkbox("Auto Play Animation")
        time_index = st.sidebar.slider("Select Time Step", 0, len(times) - 1, 0)
        ...
        #        st.empty()  # Placeholder for updating frame
        #        st.write(f"Time: {times[t_idx]}")
        #        st.rerun()

                #st.experimental_rerun()  # Re-run app for each frame
