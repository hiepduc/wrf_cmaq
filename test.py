...
def compute_pm25_from_aelmo_for_all_days(ds_apmdiag):
    ...
    for t in range(ds_apmdiag.sizes['TSTEP']):
        ...
        pm25 = ds_apmdiag["PM2.5"].isel(TSTEP=t).values
...
def plot_concentration_with_wind(ds, variable, lon2d, lat2d, time_index, wrf_ds, cmaq_time, show_wind, nx, ny):
    ...
    if show_wind:
        try:
            ...
            uu = wrf_sel["U"].isel(num_metgrid_levels=0).values
            vv = wrf_sel["V"].isel(num_metgrid_levels=0).values
...
wrf_base = st.sidebar.text_input(
    ...
    "/mnt/scratch_lustre/ar_policy/whe_project/esme_local/wrf_gmr_2013/run/WPS"
)
...
if file_list:
    try:
        ...
        if variable.lower() in ("pm2.5", "pm25", "pm_2_5", "pm-2.5"):
            ...
            if not all_apm_files:
                ...
                for t in range(len(all_aconc_files)):  # Iterate over all time steps (4 days)
                    ...
                    pm25_day = compute_pm25_from_aelmo_for_all_days(ds_apmdiag)
        ...
        wrf_files = sorted(glob.glob(os.path.join(wrf_base, f"met_em.{domain}.2013-07-2*nc")))
