...
if nc_path:
    try:
        ...
        if station_info and len(date_range) == 2:
            ...
            #ts = ds[parameter_code].isel(lat=i_lat, lon=i_lon).to_series()
            #series = ds[variable][:, 0, i, j].values
            #df_ts = pd.DataFrame({'Time': times, parameter_code: ts})

            # Optional: convert to DataFrame with datetime index
            #df_pred = ts.reset_index().rename(columns={parameter_code: "Predicted"})
            #df_pred["datetime"] = pd.to_datetime(df_pred["time"])
            #df_pred = df_pred[["datetime", "Predicted"]]

            # Predicted DataFrame
            df_pred = df_ts.set_index("Time").rename(columns={parameter_code: "pred"})
            df_pred = df_pred[(df_pred.index >= pd.to_datetime(start_date)) & (df_pred.index <= pd.to_datetime(end_date))]

            # --- Simulated observation fetch function ---
            ...
            df_obs = fetch_observation_data(
                ...
                parameter_code=api_param_code,  # <== use mapped name
            ...
            # Merge for comparison ; note that unit in CMAQ is ppm while in Aqisnet pphm
            df_merged = df_pred.join(df_obs/100, how='inner')
