import xarray as xr
import matplotlib
matplotlib.use("Agg")  # Non-interactive backend, no Tkinte
...
# Paths
cmaq_base = "/mnt/scratch_lustre/ar_policy/whe_project/esme_local/jul13_S4_fix/run/CTM"
cmaq_baseb = "/mnt/scratch_lustre/ar_policy/whe_project/esme_local/jul13_S4_barraprep/run/CTM"

mcip_base = "/mnt/scratch_lustre/ar_policy/whe_project/esme_local/jul13_S4_fix/run/MCIP"
mcip_baseb = "/mnt/scratch_lustre/ar_policy/whe_project/esme_local/jul13_S4_barraprep/run/MCIP"
...
if os.path.exists(cmaq_baseb):
    file_listb = sorted(glob.glob(
        #os.path.join(cmaq_base, f"2013-07-2*/{domain}/CCTM_ACONC_v54_intel_{domain}_*.nc")
        os.path.join(cmaq_baseb, f"2013-07-*/{domain}/CCTM_ACONC_v54_intel_{domain}_*.nc")
    ))
    print(f"Found {len(file_listb)} CMAQ files.")
...
if file_list:
    try:
        ...
        print("CMAQ file (model a)  loaded.")
        ...
        print("CMAQ model b file loaded.")
        ...
        if variable.lower() in ("pm2.5", "pm25", "pm_2_5", "pm10"):
            ...
            all_aconc_filesb = file_listb if file_listb else None
            all_apm_files = []

            for aconc_file in all_aconc_filesb:
                # Find the corresponding AELMO file for each ACONC file
                apm_file = find_apmdiag_for_aconc(aconc_file)
                if apm_file and os.path.exists(apm_file):
                    all_apm_files.append(apm_file)
            ...
            if not all_apm_files:
                ...
                for t in range(len(all_aconc_filesb)):  # Iterate over all time steps (4 days)
                    ds_aconc = xr.open_dataset(all_aconc_filesb[t]).isel(LAY=[0])
                    # Select only the first layer (LAY=0)
                    # Compute PM2.5 using ACONC and AELMO data
                    ds_apmdiag = ds_apmdiag_list[t]  # Select the corresponding AELMO dataset
                    pm25_day, pm10_day = compute_pm25_from_aelmo_for_all_days(ds_apmdiag)
                ...
                # Add the PM2.5 DataArray to the dataset
                dsb["PM2.5"] = pm25_data_array
                dsb["PM10"] = pm25_data_array
...
###########################
        ...
        for station in stations:
            ...
            for parameter_code, parameter_api_name in PARAMETER_API_MAP.items():
                try:
                    ...
                    seriesb = dsb[parameter_code][:, 0, i, j].values
                    ...
                    dfb_pred = pd.DataFrame({"pred": seriesb}, index=pd.to_datetime(times))
                    ...
                    dfb_pred = dfb_pred.tz_localize("UTC").tz_convert("Australia/Sydney")
                    dfb_pred.index.name = "datetime_local"
                    ...
                    if parameter_code.lower() in ("pm2.5", "pm25", "pm_2_5", "pm-2.5", "pm10"):
                        ...
                        dfb_merged = pd.merge(dfb_pred, df_obs, left_index=True, right_index=True, how="inner")
                    else:
                        df_merged = pd.merge(df_pred, df_obs / 100, left_index=True, right_index=True, how="inner")
                        dfb_merged = pd.merge(dfb_pred, df_obs / 100, left_index=True, right_index=True, how="inner")
                    ...
                    if not df_merged.empty:
                        ...
                        dfb_merged = dfb_merged.rename(columns={"pred": "Model", "obs": "Observed"})

                        # Plot model a prediction vs observation using Matplotlib
                        ...
                        plt.title(f"Model a vs Observed at {station_name}")
                        ...
                        # Display the plot
                        #plt.show()
                        plt.savefig(f"{parameter_code}_{station_name}_model_a.png", dpi=300)
                        plt.close()

                        # Plot model b prediction vs observation using Matplotlib
                        plt.plot(dfb_merged.index, dfb_merged['Model'], label='Model', color='blue')
                        plt.plot(dfb_merged.index, dfb_merged['Observed'], label='Observed', color='red')

                        # Add labels and title
                        plt.xlabel('Time')
                        plt.ylabel(f"{parameter_code} ({ds[parameter_code].units})")
                        plt.title(f"Model b vs Observed at {station_name}")
                        ...
                        # Display the plot
                        #plt.show()
                        plt.savefig(f"{parameter_code}_{station_name}_model_b.png", dpi=300)
                        plt.close()

                        from sklearn.metrics import mean_squared_error
                        rmse = mean_squared_error(df_merged["Observed"], df_merged["Model"], squared=False)
                        print("RMSE Model a", f"{rmse:.3f} ppm")
                        rmseb = mean_squared_error(dfb_merged["Observed"], dfb_merged["Model"], squared=False)
                        print("RMSE Model b", f"{rmse:.3f} ppm")

                        metrics = compute_metrics(df_merged.rename(columns={"pred": "Model"}))
                        metrics.update({"Station": station_name, "Parameter": parameter_code, "Model": "jul13_S4_fix"})
                        print("### 📈 Validation Metrics for Model a")
                        print(metrics)
                        results.append(metrics)

                        metricsb = compute_metrics(dfb_merged.rename(columns={"pred": "Model"}))
                        metricsb.update({"Station": station_name, "Parameter": parameter_code, "Model": "Jul13_S4barra"})
                        print("### 📈 Validation Metrics for Model b")
                        print(metricsb)
                        results.append(metricsb)

                    else:
                        print("No matching time data found between prediction and observation.")

                    # --- Compute metrics ---
                    #rmsea = np.sqrt(((df_merged["Model"] - df_merged["Observed"])**2).mean())
                    #biasa = (df_merged["Model"] - df_merged["Observed"]).mean()
                    #corra = df_merged["Model"].corr(df_merged["Observed"])
                    #rmseb = np.sqrt(((dfb_merged["Model"] - dfb_merged["Observed"])**2).mean())
                    #biasb = (dfb_merged["Model"] - dfb_merged["Observed"]).mean()
                    #corrb = dfb_merged["Model"].corr(dfb_merged["Observed"])
        ...
        rmse_matrix_A = metrics_df[metrics_df["Model"]=="jul13_S4_fix"].pivot(index="Station", columns="Parameter", values="RMSE")
        rmse_matrix_B = metrics_df[metrics_df["Model"]=="Jul13_S4barra"].pivot(index="Station", columns="Parameter", values="RMSE")
        bias_matrix_A = metrics_df[metrics_df["Model"]=="jul13_S4_fix"].pivot(index="Station", columns="Parameter", values="Mean Bias (MB%)")
        bias_matrix_B = metrics_df[metrics_df["Model"]=="Jul13_S4barra"].pivot(index="Station", columns="Parameter", values="Mean Bias (MB%)")
        corr_matrix_A = metrics_df[metrics_df["Model"]=="jul13_S4_fix"].pivot(index="Station", columns="Parameter", values="Correlation")
        corr_matrix_B = metrics_df[metrics_df["Model"]=="Jul13_S4barra"].pivot(index="Station", columns="Parameter", values="Correlation")
        merr_matrix_A = metrics_df[metrics_df["Model"]=="jul13_S4_fix"].pivot(index="Station", columns="Parameter", values="Mean Error (ME)")
        merr_matrix_B = metrics_df[metrics_df["Model"]=="Jul13_S4barra"].pivot(index="Station", columns="Parameter", values="Mean Error (ME)")

        # Difference (B - A) — negative means Model B is closer to obs
        rmse_diff = rmse_matrix_B - rmse_matrix_A
        bias_diff = bias_matrix_B - bias_matrix_A
        corr_diff = corr_matrix_B - corr_matrix_A
        merr_diff = merr_matrix_B - merr_matrix_A

        plt.figure(figsize=(10, 6))
        sns.heatmap(rmse_diff, annot=True, fmt=".2f", cmap="coolwarm", center=0)
        plt.title("RMSE Difference Heatmap (ModelB - ModelA)")
        plt.ylabel("Station")
        plt.xlabel("Parameter")
        plt.tight_layout()
        plt.savefig("rmse_diff_heatmap.png", dpi=300)
        plt.close()

        plt.figure(figsize=(10, 6))
        sns.heatmap(bias_diff, annot=True, fmt=".2f", cmap="coolwarm", center=0)
        plt.title("BIAS Difference Heatmap (ModelB - ModelA)")
        plt.ylabel("Station")
        plt.xlabel("Parameter")
        plt.tight_layout()
        plt.savefig("bias_diff_heatmap.png", dpi=300)
        plt.close()

        plt.figure(figsize=(10, 6))
        sns.heatmap(corr_diff, annot=True, fmt=".2f", cmap="coolwarm", center=0)
        plt.title("CORR Difference Heatmap (ModelB - ModelA)")
        plt.ylabel("Station")
        plt.xlabel("Parameter")
        plt.tight_layout()
        plt.savefig("corr_diff_heatmap.png", dpi=300)
        plt.close()

        plt.figure(figsize=(10, 6))
        sns.heatmap(corr_diff, annot=True, fmt=".2f", cmap="coolwarm", center=0)
        plt.title("Mean Error Difference Heatmap (ModelB - ModelA)")
        plt.ylabel("Station")
        plt.xlabel("Parameter")
        plt.tight_layout()
        plt.savefig("merr_diff_heatmap.png", dpi=300)
        plt.close()
